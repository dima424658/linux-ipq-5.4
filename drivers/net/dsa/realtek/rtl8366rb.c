...
static int rtl8366rb_setup(struct dsa_switch *ds)
{
	 ...
	 *
	 * Frames over 1500 bytes (data plus ethernet framing and
	 * checksum) does not work with CPU tagging turned on, so using
	 * this caps the MTU to 1500 bytes.
	 */
...
static int rtl8366rb_change_mtu(struct dsa_switch *ds, int port, int new_mtu)
{
	...
	ret = regmap_read(priv->map, RTL8366RB_CPU_CTRL_REG, &val);
	if (ret)
		return ret;
	/* We can't change the MTU when using CPU tagging */
	if (!(val | RTL8366RB_CPU_NO_TAG))
		return -EINVAL;
...
static int rtl8366rb_max_mtu(struct dsa_switch *ds, int port)
{
	...
	/* If CPU tagging is active, we can only handle 1500 bytes,
	 * MTU, plus ethernet framing. (The CPU tag is not included.)
	 * Observe inverted meaning of RTL8366RB_CPU_NO_TAG: CPU tagging
	 * (i.e. DSA tagging in Linux terms) is ON when this is NOT set.
	 */
	if (!(val | RTL8366RB_CPU_NO_TAG)) {
		if (dsa_is_cpu_port(ds, port))
			/* We allow the additional DSA tag to be passed in */
			return ETH_DATA_LEN + RTL8366RB_CPU_TAG_SIZE;
		else
			return ETH_DATA_LEN;
	}
